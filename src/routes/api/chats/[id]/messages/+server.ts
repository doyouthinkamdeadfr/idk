...
export const GET: RequestHandler = async ({ params, locals, platform }) => {
	...
	const { results } = await db
		.prepare(
			`SELECT id, role, content, attachments, sources, created_at FROM message WHERE chat_id = ? ORDER BY created_at ASC`
		)
		.bind(params.id)
		.all();
...
export const POST: RequestHandler = async ({ params, request, locals, platform }) => {
	...
	const chat = await db
		.prepare(`SELECT id FROM chat WHERE id = ? AND user_id = ?`)
	...
	for (const file of fileEntries) {
		...
		await db
			.prepare(
				`INSERT INTO document (id, user_id, name, type, mime, size, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'processing', ?, ?)`
			)
			.bind(
				docId,
				locals.user.id,
		...
		await db
			.prepare(`INSERT OR IGNORE INTO chat_document (chat_id, document_id) VALUES (?, ?)`)
			.bind(params.id, docId)
			.run();
	...
	for (const url of links) {
		...
		await db
			.prepare(
				`INSERT INTO document (id, user_id, name, type, mime, status, created_at, updated_at) VALUES (?, ?, ?, 'link', 'text/html', 'processing', ?, ?)`
			)
	...
	await db
		.prepare(
			`INSERT INTO message (id, chat_id, user_id, role, content, attachments, created_at) VALUES (?, ?, ?, 'user', ?, ?, ?)`
		)
	...
	const firstMsg = (await db
		.prepare(`SELECT COUNT(*) as cnt FROM message WHERE chat_id = ?`)
		.bind(params.id)
		.first()) as any;
	if (firstMsg?.cnt === 1) {
			...
			try {
				if (text.trim() && smDocIds.length > 0) {
					...
					await Promise.allSettled(
						smDocIds.map((id) => waitForProcessing(id, platform).catch(() => {}))
					);
				}
				...
				let fullContent = '';
				await chatComplete(messages as any, platform, (token) => {
					fullContent += token;
				...
				await db
					.prepare(
						`INSERT INTO message (id, chat_id, user_id, role, content, sources, created_at) VALUES (?, ?, ?, 'assistant', ?, ?, ?)`
					)
					.bind(
						msgId,
						params.id,
				...
				if (chatInfo?.title === 'New Chat' && text.trim()) {
					...
					await db
						.prepare(`UPDATE chat SET title = ? WHERE id = ?`)
